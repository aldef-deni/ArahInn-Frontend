...
const STATUS_META = {
  pending     : { labelKey: 'ppob.statusPending',    color: 'amber',   Icon: Clock },
  paid        : { labelKey: 'ppob.statusPaid',       color: 'blue',    Icon: Clock },
  processing  : { labelKey: 'ppob.statusProcessing', color: 'blue',    Icon: Clock },
  success     : { labelKey: 'ppob.statusSuccess',    color: 'emerald', Icon: CheckCircle },
  failed      : { labelKey: 'ppob.statusFailed',     color: 'red',     Icon: XCircle },
  refundable  : { labelKey: 'ppob.statusRefundable', color: 'amber',   Icon: RotateCcw },
  refunded    : { labelKey: 'ppob.statusRefunded',   color: 'slate',   Icon: RotateCcw },
  canceled    : { labelKey: 'ppob.statusCanceled',   color: 'slate',   Icon: XCircle },
}
  ...
  if (trxList.length === 0) {
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-10 sm:p-12 text-center">
        ...
        <p className="text-slate-700 font-semibold">{t('ppob.noTrx')}</p>
        <p className="text-xs text-slate-500 mt-1">{t('ppob.noTrxDesc')}</p>
        {emptyCta && (
      ...
      {trxList.map(trx => {
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-slate-500 mt-1">
                {t('ppob.toCustomer')} <span className="font-mono">{customer}</span>
              </p>
            ...
            <div className="text-right shrink-0">
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">
                {createdAt
                  ? new Date(createdAt).toLocaleString(i18n.language === 'en' ? 'en-US' : 'id-ID', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })
                  : '-'}
              </p>
            </div>
          </div>
        ...
        const StrukBtn = isSuccess && trxCode ? (
          <button onClick={(e) => downloadStruk(e, trxCode)} disabled={isDownloading}
            ...
            {isDownloading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />} {t('ppob.downloadReceiptPdf')}
          </button>
        ) : null
