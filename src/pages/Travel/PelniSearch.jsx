    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-sm sm:p-4" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full sm:max-w-md max-h-[85vh] bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden">
        <div className="shrink-0 px-4 pt-3 pb-3 border-b border-slate-100">
          ...
          <div className="relative"><Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" /><input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Cari pelabuhan..." className="w-full pl-9 pr-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-300" /></div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 && <p className="text-center text-sm text-slate-400 py-8">Pelabuhan tidak ditemukan</p>}
          {filtered.map(p => (
...
export default function PelniSearch() {
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      const res = await travelApi.searchPelni({
        ...
        startDate: date, endDate: end.toISOString().slice(0, 10),
      })
      setResults(res.data?.data || [])
      setShowForm(false)
      setTimeout(() => resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100)
    } catch (e) { setError(e?.response?.data?.message || 'Gagal mencari jadwal kapal.') }
  ...
  return (
    <div className="min-h-[70vh] bg-slate-50">
      <SEO title="Tiket Kapal Laut PELNI — Cari Jadwal" description="Pesan tiket kapal PELNI semua rute Indonesia di ArahInn." url="/tiket/pelni" />
      ...
      <section className="bg-gradient-to-br from-cyan-500 to-blue-600 text-white">
        <div className="container py-5 sm:py-7">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div><h1 className="font-display text-lg sm:text-xl font-bold leading-tight">Tiket Kapal Laut</h1><p className="text-[11px] sm:text-xs text-white/80">PELNI — semua rute Indonesia</p></div>
          </div>
          <div className="bg-white rounded-2xl shadow-lg p-3.5 sm:p-4 text-slate-900">
            <div className="relative grid grid-cols-1 gap-2.5">
              <button onClick={() => setPicker('origin')} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 hover:border-cyan-400 text-left"><MapPin className="w-4 h-4 text-slate-400 shrink-0" /><div className="min-w-0 flex-1"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Dari</p><p className={`text-sm font-semibold truncate ${origin ? 'text-slate-900' : 'text-slate-400'}`}>{origin ? `${titleCase(origin.NAME)} (${origin.CODE})` : 'Pilih pelabuhan asal'}</p></div></button>
              <button onClick={() => setPicker('destination')} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 hover:border-cyan-400 text-left"><MapPin className="w-4 h-4 text-slate-400 shrink-0" /><div className="min-w-0 flex-1"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Ke</p><p className={`text-sm font-semibold truncate ${destination ? 'text-slate-900' : 'text-slate-400'}`}>{destination ? `${titleCase(destination.NAME)} (${destination.CODE})` : 'Pilih pelabuhan tujuan'}</p></div></button>
            ...
            <div className="grid grid-cols-2 gap-2.5 mt-2.5">
              <div className="relative p-3 rounded-xl border border-slate-200 cursor-pointer" onClick={openDatePicker}>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide flex items-center gap-1"><Calendar className="w-3 h-3" /> Tgl Berangkat (±14 hari)</p>
            ...
            <button onClick={doSearch} disabled={!canSearch || searching} className="w-full mt-3 py-3.5 rounded-xl bg-cyan-500 hover:bg-cyan-600 text-white font-bold text-sm flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-50">{searching ? <><Loader2 className="w-4 h-4 animate-spin" /> Mencari jadwal kapal...</> : <><Search className="w-4 h-4" /> Cari Kapal</>}</button>
          </div>
        </div>
      </section>
      )}
      ...
      <section ref={resultsRef} className="container py-5">
        ...
        {results && results.length > 0 && (
          <>
            <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-3 flex items-center justify-between gap-3">
              <div className="min-w-0"><p className="font-bold text-slate-900 text-base sm:text-lg flex items-center gap-2 flex-wrap leading-tight">{titleCase(origin?.NAME)} <ArrowRight className="w-4 h-4 text-slate-400 shrink-0" /> {titleCase(destination?.NAME)}</p><p className="text-xs text-slate-500 mt-1">{results.length} jadwal · {adult} Dewasa{child>0?`, ${child} Anak`:''}{infant>0?`, ${infant} Bayi`:''}</p></div>
              <button onClick={() => { setShowForm(true); window.scrollTo({ top: 0, behavior: 'smooth' }) }} className="shrink-0 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold">Ubah</button>
            </div>
                    ...
                    {(ship.fares || []).map((fare, j) => {
                      ...
                      return (
                        <div key={j} className="flex items-center justify-between gap-2">
                          <div><p className="text-sm font-semibold text-slate-900">{classLabel(fare.CLASS)} <span className="text-[10px] text-slate-400">({fare.SUBCLASS})</span></p><p className={`text-[10px] font-semibold ${soldOut ? 'text-red-500' : 'text-emerald-600'}`}>{soldOut ? 'Penuh' : `Sisa ${avail} (P${fare.AVAILABILITY?.M}/W${fare.AVAILABILITY?.F})`}</p></div>
                          <div className="text-right">
                            <p className="font-display text-sm font-bold text-cyan-600">{formatRupiah(harga + markup)}<span className="text-[9px] text-slate-400">/pax</span></p>
                            <button onClick={() => selectClass(ship, fare)} disabled={soldOut} className="mt-1 px-4 py-1.5 rounded-full bg-cyan-500 hover:bg-cyan-600 text-white text-xs font-bold disabled:opacity-40 disabled:bg-slate-300">Pilih</button>
                          </div>
                        </div>
                      )
                    })}
        ...
        {results && results.length === 0 && !searching && <div className="text-center py-12"><Ship className="w-12 h-12 text-slate-300 mx-auto mb-3" /><p className="font-semibold text-slate-700">Tidak ada jadwal kapal</p><p className="text-sm text-slate-400 mt-1">Coba ubah tanggal atau rute. Jadwal Pelni mingguan.</p></div>}
        {!results && !searching && <div className="text-center py-12"><Ship className="w-12 h-12 text-slate-200 mx-auto mb-3" /><p className="text-sm text-slate-400">Isi rute & tanggal, lalu cari jadwal kapal PELNI.</p></div>}
      </section>

      <PortPicker open={picker === 'origin'} ports={origins} title="Pelabuhan Asal" onPick={setOrigin} onClose={() => setPicker(null)} />
      <PortPicker open={picker === 'destination'} ports={destinations} title="Pelabuhan Tujuan" onPick={setDestination} onClose={() => setPicker(null)} />
    </div>
  )
}
